...
1. **aggregator** — 过滤数据（量、涨幅范围、OI 基准），输出候选列表
2. **AI（Claude）** — 读取候选，输出信号倾向（`BUY_LIKELY` / `SKIP`），journal 作状态参考
3. **decide.py** — 唯一最终决策入口：AI 倾向 + 硬规则（价格/OI/回调阈值）→ open/skip
4. **execute.py** — 纯风控执行：仓位计算、止损、日亏限制，不参与策略判断
...
| 日亏软限 | 当日亏损超净值 20% | 仓位缩减至 50% |
| 日亏严格 | 当日亏损超净值 40% | 仓位缩减至 25% |
| 日亏硬限 | 当日亏损超净值 60% | 禁止新开仓（平仓不受影响）|
| 最大持仓 | 同时最多 2 个 | 分散风险 |
